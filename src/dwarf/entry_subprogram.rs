...
use gimli::{AttributeValue, DW_AT_abstract_origin, DW_AT_inline, DW_AT_linkage_name, DW_AT_low_pc, DW_AT_specification, DW_AT_virtuality, DW_AT_vtable_elem_location, DW_INL_declared_inlined, DW_INL_declared_not_inlined, DW_INL_inlined, DW_INL_not_inlined, DW_VIRTUALITY_none, DW_VIRTUALITY_pure_virtual, DW_VIRTUALITY_virtual};
...
impl<'d, 'i> UnitCtx<'d, 'i> {
    ...
    pub fn get_entry_vtable_elem_location(&self, entry: &DIE<'i, '_, '_>) -> Result<Option<usize>, Error> {
        ...
        match virtuality {
            None | Some(AttributeValue::Virtuality(DW_VIRTUALITY_none)) => {
                ...
                if vel.is_some() {
                    return bad!(self, offset, Error::UnexpectedEntryAttr(DW_AT_vtable_elem_location));
                }
                Ok(None)
            }
            ...
            _ => {
                return bad!(self, offset, Error::BadEntryAttrType(DW_AT_virtuality, "Virtuality"));
            }
        }
    }

    /// Get the DW_AT_linkage_name of a DIE, allowing it to be missing
    pub fn get_entry_linkage_name(&self, entry: &DIE<'i, '_, '_>) -> Result<Option<&'i str>, Error> {
        let offset = self.to_global_offset(entry.offset());
        let name = err_ctx!(
            self,
            offset,
    ...
    pub fn get_entry_abstract_origin(&self, entry: &DIE<'i, '_, '_>) -> Result<Option<UnitOffset>, Error> {
        let offset = self.to_global_offset(entry.offset());
        let origin = err_ctx!(
            self,
            offset,
        ...
        match origin {
            ...
            _ => bad!(self, offset, Error::BadEntryAttrType(DW_AT_abstract_origin, "UnitRef"))
                .attach_printable(format!("Got: {:?}", origin)),
        }
    }

    /// Get the DW_AT_specification of a DIE, allowing it to be missing
    pub fn get_entry_specification(&self, entry: &DIE<'i, '_, '_>) -> Result<Option<UnitOffset>, Error> {
        let offset = self.to_global_offset(entry.offset());
        let origin = err_ctx!(
            self,
            offset,
        ...
        match origin {
            None => Ok(None),
            Some(AttributeValue::UnitRef(offset)) => Ok(Some(offset)),
            _ => bad!(self, offset, Error::BadEntryAttrType(DW_AT_specification, "UnitRef"))
                .attach_printable(format!("Got: {:?}", origin)),
        }
    }
    ...
    pub fn get_entry_inlined(&self, entry: &DIE<'i, '_, '_>) -> Result<bool, Error> {
        ...
        match inline {
            None | Some(AttributeValue::Inline(DW_INL_not_inlined))
            ...
            _ => bad!(self, offset, Error::BadEntryAttrType(DW_AT_virtuality, "Virtuality"))
