...
fn main_inner() -> Result<(), Error> {
    ...
    let decompiled_functions =
        uking::read_uking_functions("botw-decomp/data/uking_functions.csv", &mut uking_symbols)
    ...
    let mut dwarf = dwarf::extract(elf_path, &mut uking_symbols, &decompiled_functions)
    ...
    for (i, (name, info)) in dwarf.address.into_iter().enumerate() {
        ...
        let info = info
            .into_def(&dwarf.types, &type_defs)
